...
"""
input_txt = "".join(input_txt.split())  # remove all whitespaces

solution = ""

# solutions at the very beginning and end would not be found, but in this case it is good enough
idx = 4
while idx < (len(input_txt) - 4):
    if input_txt[idx].islower():
        if input_txt[(idx-3):idx].isupper() and input_txt[(idx+1):(idx+4)].isupper() \
                and input_txt[idx-4].islower() and input_txt[idx+4].islower():
            solution += input_txt[idx]
    idx += 1
print(solution)